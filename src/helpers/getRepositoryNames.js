...
// Returns an array containing the objects with name and namespace
// of the repository relative to the source directory
module.exports = async (source, verbose) => {
  try {
    const pathsToRepositories = await getPathsToRepositories(source, verbose);

    const result = await Promise.all(pathsToRepositories.map(repositoryPath => (
